...
static void psa_setup_byte_buffer(uint8_t* buffer, uint64_t key1, uint64_t key2);
...
static void psa_build_buffer_mode23(
    ...
    bool has_original_key2 = (instance->generic.data_2 != 0);
    if(has_original_key2) {
        original_buffer9 = (uint8_t)(instance->generic.data_2 & 0xFF);
        original_buffer8 = (uint8_t)((instance->generic.data_2 >> 8) & 0xFF);
        buffer[9] = original_buffer9;
        FURI_LOG_D(
            TAG,
            "Original Key2: 0x%04X, buffer[8]=0x%02X buffer[9]=0x%02X, preserving buffer[9]",
            (unsigned int)(instance->generic.data_2 & 0xFFFF),
            original_buffer8,
            original_buffer9);
    } else {
...
static void psa_encoder_build_upload(SubGhzProtocolEncoderPSA* instance) {
    ...
    if(instance->generic.data != 0) {
        ...
        psa_setup_byte_buffer(orig_buffer, instance->generic.data, instance->generic.data_2);
        preserve_buffer01[0] = orig_buffer[0];
        preserve_buffer01[1] = orig_buffer[1];
        preserve_ptr = preserve_buffer01;
        FURI_LOG_D(
            TAG,
            "Original Key1: %016llX, preserving buffer[0-1]: %02X %02X",
...
SubGhzProtocolStatus
    ...
    do {
        if(!flipper_format_read_string(flipper_format, "Key", temp_str)) {
            ...
            uint32_t key1_low = ((uint32_t)hex_buffer[0] << 24) | ((uint32_t)hex_buffer[1] << 16) |
                                ((uint32_t)hex_buffer[2] << 8) | (uint32_t)hex_buffer[3];
            uint32_t key1_high = ((uint32_t)hex_buffer[4] << 24) |
                                 ((uint32_t)hex_buffer[5] << 16) | ((uint32_t)hex_buffer[6] << 8) |
                                 (uint32_t)hex_buffer[7];
            instance->generic.data = ((uint64_t)key1_high << 32) | key1_low;
        } else {
        ...
        if(flipper_format_read_string(flipper_format, "Key_2", temp_str)) {
            ...
            if(flipper_format_read_uint32(flipper_format, "ValidationField", &val_field_val, 1)) {
                ...
                instance->generic.data_2 = (uint64_t)instance->validation_field;
            } else {
                uint8_t val_field[2];
        ...
        instance->generic.seed = (uint8_t)(seed_val & 0xFF);
        ...
        FURI_LOG_I(TAG, "Key1: %016llX", (unsigned long long)instance->generic.data);
        FURI_LOG_I(
            TAG,
...
static void psa_setup_byte_buffer(uint8_t* buffer, uint64_t key1, uint64_t key2) {
    for(int i = 0; i < 8; i++) {
        int shift = i * 8;
        buffer[7 - i] = (uint8_t)((key1 >> shift) & 0xFF);
    }
    buffer[9] = (uint8_t)(key2 & 0xFF);
    buffer[8] = (uint8_t)((key2 >> 8) & 0xFF);
}
...
static void psa_extract_fields_mode23(uint8_t* buffer, SubGhzProtocolDecoderPSA* instance) {
    instance->generic.btn = buffer[8] & 0xF;
    instance->generic.serial = ((uint32_t)buffer[3] << 8) | ((uint32_t)buffer[2] << 16) |
                               (uint32_t)buffer[4];
    instance->generic.cnt = (uint32_t)buffer[6] | ((uint32_t)buffer[5] << 8);
...
static void psa_extract_fields_mode36(uint8_t* buffer, SubGhzProtocolDecoderPSA* instance) {
    instance->generic.btn = (buffer[5] >> 4) & 0xF;
    instance->generic.serial = ((uint32_t)buffer[3] << 8) | ((uint32_t)buffer[2] << 16) |
                               (uint32_t)buffer[4];
    instance->generic.cnt = ((uint32_t)buffer[7] << 8) | ((uint32_t)buffer[6] << 16) |
...
static void psa_decrypt_router(SubGhzProtocolDecoderPSA* instance) {
    ...
    uint32_t key1_high = (uint32_t)(instance->generic.data >> 32);
    uint32_t key1_low = (uint32_t)instance->generic.data;
    FURI_LOG_I(
        TAG,
        "Key1:%08lX%08lX Key2:%04X mode_serialize:0x%02X decrypted:0x%02X",
        (unsigned long)key1_high,
        (unsigned long)key1_low,
        (unsigned int)(instance->generic.data_2 & 0xFFFF),
    ...
    psa_setup_byte_buffer(buffer, instance->generic.data, instance->generic.data_2);
    ...
    case PSADecoderState2:
        ...
        if(level && instance->decode_count_bit == PSA_KEY2_BITS) {
            if(duration >= 800) {
                ...
                if(end_diff <= 199) {
                    if(((instance->generic.data >> 48) & 0xF) != 0xA) {
                        instance->decode_data_low = 0;
                        instance->decode_data_high = 0;
                        instance->decode_count_bit = 0;
                        new_state = PSADecoderState0;
                    ...
                    instance->generic.data_2 = ((uint64_t)instance->decode_data_high << 32) |
                                               new_key2_low;
                    ...
                    psa_setup_byte_buffer(
                        buffer, instance->generic.data, instance->generic.data_2);
                    if(psa_direct_xor_decrypt(instance, buffer)) {
                        instance->mode_serialize = 0x23;
                        instance->decrypted = 0x50;
                        FURI_LOG_I(
        ...
        if(should_process && instance->decode_count_bit < PSA_KEY2_BITS) {
            ...
            if(manchester_advance(
                ...
                if(instance->decode_count_bit == PSA_KEY1_BITS) {
                    instance->generic.data = ((uint64_t)instance->decode_data_high << 32) |
                                             instance->decode_data_low;
                    instance->decode_data_low = 0;
                    instance->decode_data_high = 0;
                }
            }
...
SubGhzProtocolStatus subghz_protocol_decoder_psa_serialize(
    ...
    do {
        ...
        char key1_str[20];
        snprintf(
            key1_str, sizeof(key1_str), "%016llX", (unsigned long long)instance->generic.data);
        if(!flipper_format_write_string_cstr(flipper_format, "Key", key1_str)) break;

        char key2_str[20];
        snprintf(
            key2_str, sizeof(key2_str), "%016llX", (unsigned long long)instance->generic.data_2);
        ...
        if(instance->decrypted == 0x50 && instance->decrypted_type != 0) {
            ...
            flipper_format_write_uint32(flipper_format, "Seed", &instance->generic.seed, 1);
        }
...
void subghz_protocol_decoder_psa_get_string(void* context, FuriString* output) {
    ...
    if(instance->status_flag == 0x80 && (instance->generic.data != 0) &&
       instance->decrypted_type == 0) {
    ...
    uint16_t key2_value = (uint16_t)(instance->generic.data_2 & 0xFFFF);
